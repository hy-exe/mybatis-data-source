package com.mybatis.data.source.utils;
...
public class FieldUtil {
  ...
  static public Class<?> getComponentClass(Field field) {
    ...
    if (null == type || !(ParameterizedType.class.isInstance(type))) {
      String errmsg = "FieldUtils: getGenericType invalid, can't get compoment class." + "/ cause field is [" + field + "]";
      logger.error(errmsg);
      throw new RuntimeException(errmsg);
    }
  ...
  static public Field[] getAnnotationFieldsOf(Class<?> cls, Class<? extends Annotation> annotationClass) {
    Field[] fields = new Field[0];
