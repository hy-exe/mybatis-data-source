...
import com.mybatis.data.source.utils.FieldUtil;
...
public class IbatisQueryDelegate implements QueryDelegate {
  ...
  @Override
  public List<Object> queryForPagedList(String countStatement, String listStatement, Object param, Paginator paginator, DBRoute dr) {
    Map<String, SqlSession> dbMap = dbRouteConfig.getSqlMapTemplates(dr, countStatement);

    if (dbMap.size() != 1) {
      throw new RuntimeException("more than 1 database found, please confirm the parameters. DBRoute=[" + dr + "], countStatement=[" + countStatement
  ...
  @Override
  public List<Object> queryForMergedList(String statementName, Object parameterObject, Paginator paginator, String orderByString, DBRoute dr) {

    List<Object> list = queryForList(statementName, parameterObject, dr);
    ...
    if (!orderByFields.isEmpty()) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(statementName + " Merge Sort Run time estimated: " + (endTime - startTime) + "ms");
      }
    }
